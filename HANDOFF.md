...
(Bug 4 and Bug 5, formerly listed here, are fixed — see COMPLETED SINCE HANDOFF below. Bugs 9, 10 and 11 are also fixed but their entries are retained in place below, marked FIXED, because the buffer/channel/disk-boundary detail they record is still worth reading; Bugs 6, 7 and 8 remain open.)
...
**Nuance:** for a while after this fix, scenario 3 still failed its `saved file present on D64` assertion — that was Bug 11 below, not a Bug 10 regression, and the channel fix was independently verified by the D64 string checks above, which do not depend on the save succeeding end-to-end. Bug 11 has since been fixed too and scenario 3 now passes 12/12, so a reappearance of the status strings in a saved image is the signal that *this* fix has regressed.

**Bug 11 — REU save off-by-one at the disk-full boundary, and a false "SAVE COMPLETE" report (discovered 2026-07-28 while verifying the Bug 10 fix, which had been masking it; FIXED 2026-07-28, verified on VICE):** the write loop's bound was `blocks_written < disk_free`, so it wrote exactly `disk_free` full blocks. That was one too many. A 1541 allocates the *next* block whenever a buffer fills, because the current sector's link field has to point somewhere — so writing the 73rd block into 73 free blocks demands a 74th. The drive returned error 72 DISK FULL, abandoned the file, and never finalized the directory entry, leaving the D64 with an unclosed "splat" entry (type `$01`, bit 7 clear, name `REUSAVE`, blocks 0) that persisted across a clean drive detach and a clean VICE exit. The BAM confirmed free 73 → 0 with a 73-block data chain, and reading the drive error channel from BASIC after the save returned `72 DISK FULL 0 0`. Control experiment: aborting with SPACE at 28 blocks, short of the boundary, closed the file perfectly (type `$81` CLOSED, blocks 28, listed correctly), so `close_save_file` and the CHROUT path were healthy — only the boundary case broke.

**The second half of this bug was arguably worse than the first.** `write_block_to_file` only tests ST bits `$82` — device-not-present and IEC timeout. Error 72 never appears in ST at all; it exists only on the drive's error channel, which the save path never read. So the program printed "SAVE COMPLETE. 73 BLOCKS WRITTEN." for a save that had in fact **failed**. That false success was timing-dependent rather than universal: in a 664-block run the same boundary happened to trip ST and did print "WRITE ERROR!".

**Status: FIXED — both halves, since either alone would leave the program able to claim success for a failed save.**

- **One-block reserve.** After the existing free-space check, `save_limit_lo`/`save_limit_hi` are computed as `disk_free - 1` (16-bit `sec`/`sbc #1` then `sbc #0`) at `src/reu_advanced.s:580-586`, and the write loop now compares `blocks_written` against `save_limit` instead of `disk_free` (`:654-663`). The every-16-blocks progress line reports against `save_limit` too (`:752-758`), so "n OF m BLOCKS" matches where the loop actually stops; the earlier "FREE: n BLOCKS" message still reports the drive's true free count. A disk with exactly one free block is now refused as no-space (`:588-592`), that block being entirely reserve.
- **Drive error-channel check.** New routine `read_drive_error` (`src/reu_advanced.s:996`) opens the command channel (LF 15), reads the two ASCII error digits into `save_err_tens`/`save_err_ones`, and returns carry clear only when the drive reports "00"; a failure to open or read the channel is itself reported as an error, and the digits default to `'?'` so a display never shows stale values. `@write_complete` calls it after `close_save_file` and branches to the new `@write_drive_error` path (`:773-774`, `:813-827`), which prints `reu_write_error_msg` followed by the drive's own two-digit code instead of "SAVE COMPLETE".

Two implementation notes worth keeping. `read_drive_error` lives in the **`LIB_AES256ECDSA_HICODE` segment at `$7C00`** (`src/reu_advanced.s:994`, returning to `LIB_AES256ECDSA_CODE` at `:1054`), not MAIN — adding it overflowed MAIN by 88 bytes, and relocating cold-path code to the high-memory overflow area is this project's established technique for that (see `build_ca65/linker.cfg` and the README's module-ordering note). Separately, two relative branches in the write loop had to become short-branch-over-`jmp` because the routine's growth pushed them past ±127: the `write_block_to_file` error exit (`:718-720`) and the progress-interval test (`:731-733`). That idiom was already in use elsewhere in the file.

Verified on a real VICE run: `tools/test_reu.py` is now **32/32** (was 31/32), with scenario 3 "REU save-to-disk" at 12/12. The previously failing `saved file present on D64` assertion passes, and the directory shows a proper closed SEQ entry — `reusave, blocks=72` against a disk reporting 73 free, so the one-block reserve is directly observable in the output. Bug 10's status strings ("WRITING TO DISK", "PROGRESS", "REFILLING") are confirmed still absent from the image, so that fix did not regress. `read_drive_error`'s OK path is exercised by every successful save: scenario 3 reaching "SAVE COMPLETE" proves the routine opened LF 15, read two digits, matched "00" and returned carry clear, since any failure to open or parse returns carry set and would have printed "WRITE ERROR!" and failed the assertion instead. What remains uncovered is its non-"00" branch — a drive error reported only on the command channel and not in ST. A directory-full experiment (144 entries, 56 blocks free) did not reach it: the save printed "WRITE ERROR!" with no trailing digit pair, meaning the pre-existing `write_block_to_file` ST check caught it and routed to `@write_error_close`. That is correct behavior with no false success, but it leaves the channel-only error case unprovoked, and the one-block reserve now prevents the disk-full scenario that originally produced one. Regression coverage remains `tools/test_reu.py` scenario 3, which now passes in full; a regression would surface as `saved file present on D64` failing again.
