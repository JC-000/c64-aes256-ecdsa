...
Beyond the "demonstration only, not cryptographically secure" caveat above, one functional defect is worth knowing before you rely on a feature. `HANDOFF.md` carries the full known-bug list with reproduction details.

- **REU save-to-disk fails when it fills the disk exactly.** The write loop is bounded by the drive's free-block count, but a 1541 allocates the *next* block whenever a buffer fills (the current sector's link field has to point somewhere), so writing N blocks into N free blocks needs N+1. The drive returns error 72 DISK FULL, abandons the file, and leaves an unclosed directory entry. Compounding this, the save path only inspects the KERNAL `ST` byte, and error 72 never appears there — it exists only on the drive's error channel, which is never read — so the program can print "SAVE COMPLETE" for a save that failed. Saves that stop short of the disk-full boundary (including an aborted save) close correctly and are intact.
