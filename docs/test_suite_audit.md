...
**Update (2026-07-28): all ten "Top 10, Ordered by Real-World Risk" items are now closed** — see the second addendum at the end of this document for what was built, what it found, and which lower-severity findings remain open. Closing them surfaced three real program bugs that static analysis had not (`HANDOFF.md` Bugs 9, 10 and 11); all three are now fixed and verified.
...
## Second addendum: closing the Top 10 (2026-07-28)

All ten "Top 10, Ordered by Real-World Risk" items above are now closed. Item 10 was already fixed in the 2026-07-20 harness-compatibility pass; the other nine were addressed in this round. The work was split across parallel agents by lane, then every result was re-verified against live VICE before being accepted.

**What was built**

- `tools/test_disk_io.py` (8 tests) — closes items 1 and 4. Real D64 via the harness's `DiskImage`/`c1541`: key save (menu 5/7) captures C64 memory, saves, asserts the on-device "VERIFICATION OK", then reads the D64 host-side and compares bytes; overwrite handling asserts a second default-name save produces `AESKEY0` alongside `AESKEY` with correct, distinct contents; load (menu 6/8) authors files host-side and asserts C64 memory matches after loading.
- `tools/test_reu.py` (32 tests, all passing) — closes items 7 and 8. Three scenarios driving menu `G`: REU absent (asserts `reu_present`/`reu_size_kb` are zero and the UI reports NOT PRESENT), REU present at 512 KB (status, zero-fill, random-fill), and REU random-fill followed by save-to-disk. VICE is given `-reu -reusize <KB>` through `ViceConfig(extra_args=...)`; no harness change was needed.
- `tools/test_reu_disk_write.py` (2 tests) — closes item 2 (Bug 5 regression). Deterministic opcode-level check that `write_block_to_file`'s fatal status mask is exactly `AND #$82` and that the regressed `AND #$80` form is absent, plus an end-to-end save to an unattached drive asserting the error surfaces with no false success.
- `tools/check_boot_position.py` + a `check-boot` Makefile target that `all` depends on — closes item 3. Asserts the BASIC stub lands at file offset 2 and cross-checks `build/labels.txt`, so the `boot.o` link-position invariant is machine-enforced rather than comment-enforced.
- `build()` in `tools/run_all_tests.py` now fails on assembler/linker warnings even when the build exits 0 — closes item 4's `.importzp` half. Six long-standing benign `Didn't use zeropage addressing` warnings are allowlisted by file and symbol; every other warning fails the build.
- Depth cases added to the existing suites — closes items 5, 6 and 9: wrong-key AES-CBC decrypt (ordered correct-then-wrong, so a stale key schedule is caught), GCM-SIV ciphertext tampering as well as tag tampering, GCM-SIV wrong-key decrypt, and out-of-range length fault injection on both encryption paths.
- `tools/supervise_gcmsiv_tests.py` re-labelled as historical/bootstrap-only, per the addendum's suggested action.

**What it found**

Exercising these paths for the first time surfaced three real program bugs that source inspection had missed. Full detail lives in `HANDOFF.md`.

- **Bug 9 (now fixed)** — neither encryption path bounded its length byte. The UI clamps at 63 characters, so only direct-memory callers could reach it; over-length input overran the output buffer into adjacent state and, past ~200 bytes, corrupted the block loop's own control variables. Guards now live inside the routines, rejecting rather than truncating.
- **Bug 10 (now fixed)** — REU save-to-disk left the open disk file selected as the CHROUT output channel, so progress messages were written *into the saved file*. This corrupted the data and overflowed the exactly-sized free-block budget, failing every save regardless of disk size. The status strings were recoverable from the resulting D64 with `strings(1)`, which is what made it unambiguous.
- **Bug 11 (now fixed)** — masked by Bug 10 until that fix landed. The write loop was bounded by the free-block count, but a 1541 needs one more block than it writes (for the sector link field), so a save that exactly filled the disk hit error 72 and was abandoned. Error 72 never appears in the KERNAL `ST` byte and the drive error channel was never read, so the program reported "SAVE COMPLETE" for a failed save. Fixed in both halves: the write loop now reserves one block against the free count, and the save reads the drive's command channel before claiming success, reporting the drive's own error code when it fails. The reserve is directly observable in the test output — a disk reporting 73 free now yields a closed 72-block file.

  One caveat on the second half's coverage. The command-channel routine's success path is exercised by every passing save (a save only prints "SAVE COMPLETE" if the routine opened the channel, read two digits and got "00"), but its *error* branch is not covered by a provoked channel-only failure. A directory-full disk was tried and turned out to trip `ST` instead, so it is caught by the older check and never reaches the new one — correct behaviour, but not a test of that branch. The scenario that originally produced a channel-only error is now unreachable by construction, since the reserve prevents it.

Note that the three bugs left their regression tests in two different polarities, which is easy to get backwards when reading the suites: the Bug 9 fault-injection tests were rewritten to assert the guard (they previously passed against the unguarded behaviour), whereas `test_reu.py` asserted correct behaviour throughout and simply went green as Bugs 10 and 11 were fixed — 29/31, then 31/32, then 32/32. The denominator moves because assertions nested inside the success path only become reachable once the save actually succeeds.

**Still open**

The 22 lower-severity findings in the main body are untouched, as are the honorable mentions — `make lib`/`make lib-ecdsa-sign` archive automation, and UI coverage for `do_benchmark`, SID config and the RNG stream display. Two test-infrastructure gaps are worth flagging because they bite anyone running suites concurrently: `tools/test_aes_cbc_direct.py` and `tools/test_gcmsiv_encrypt_direct.py` honour neither `C64_SKIP_BUILD` nor `C64_PORT_RANGE_START` and unconditionally run `make clean && make`, so running either standalone destroys build artifacts other suites are using. `tools/test_reu.py` also has known depth limits: it never reads the saved file back to verify content (which would have caught Bug 10 directly rather than via `strings`), never verifies the REU actually holds the pattern it was filled with, and silently shrinks its own denominator on early-failure paths. Any future content-level assertion against a D64 must detach the drive first — VICE leaves the tail of the image unflushed otherwise.
