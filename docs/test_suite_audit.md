...
**Update (2026-07-28): all ten "Top 10, Ordered by Real-World Risk" items are now closed** — see the second addendum at the end of this document for what was built, what it found, and which lower-severity findings remain open. Closing them surfaced three real program bugs that static analysis had not: two are now fixed (`HANDOFF.md` Bugs 9 and 10) and one is newly documented and open (Bug 11).
...
- **Bug 11 (open)** — masked by Bug 10 until that fix landed. The write loop is bounded by the free-block count, but a 1541 needs one more block than it writes (for the sector link field), so a save that exactly fills the disk hits error 72 and is abandoned. Error 72 never appears in the KERNAL `ST` byte and the drive error channel is never read, so the program can report "SAVE COMPLETE" for a failed save.

Note the two fixed bugs left their regression tests in opposite polarities, which is easy to get backwards when reading the suites: the Bug 9 fault-injection tests were rewritten to assert the guard and pass today, while `test_reu.py` scenario 3 asserts correct behaviour and still fails on Bug 11.
